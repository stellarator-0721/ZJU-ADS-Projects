/* This is the Porter stemming algorithm, coded up as thread-safe ANSI C
   by Adam Cooper. It's copyright but you can do whatever you like with it.
   It has web-based interfaces at http://www.wooorm.com/stmr.c
*/
...
/* The main part of the stemming algorithm starts here. b is a buffer
   holding a word to be stemmed. The letters are in b[i] ... b[j]. k is a
   cursor into the buffer, pointing to the end of the current word. I is
   for measuring the length of the word.
   Algorithm by Martin Porter, http://www.cs.odu.edu/~jbollen/IR/stems.html
*/
static char * b;
static int k, k0, j;
...
/* In stem(p,i,j), p is a char pointer, and the string to be stemmed is from
   ...
   Stemming never increases word length, so i is not changed.
*/
int stem(char * p, int i, int j) {
    b = p; k = j; k0 = i; /* copy the parameters into statics */
    if (k <= k0 + 1) return k; /*-DEPARTURE FROM PUBLISHED ALGORITHM*/

    /* With this line, strings of length 1 or 2 don't go through the
       stemming process, although no mention is made of this in the
       published algorithm. Remove the line to stem words like 'an' and 'of'
       and 'in' and 'it' to 'a', 'o', 'i', and 'i'. */
    step1ab();
    step1c();
    step2();
    step3();
